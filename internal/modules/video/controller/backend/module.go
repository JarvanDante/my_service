...
func moduleList(ctx context.Context, mod service.IModule, name, position, status string, categoryId int64, page, size int) ([]v1.ModuleItem, int, error) {
	list, total, err := mod.List(ctx, service.ModuleFilter{
		Name: name, Position: position, CategoryId: categoryId, Status: parseOptionalInt(status),
		Page: page, Size: size,
	})
	if err != nil {
		return nil, 0, err
...
func (c *Controller) VideoModuleList(ctx context.Context, req *v1.VideoModuleListReq) (res *v1.VideoModuleListRes, err error) {
	list, total, err := moduleList(ctx, c.videoMod, req.Name, req.Position, req.Status, req.CategoryId, req.Page, req.Size)
	if err != nil {
		return nil, err
	}
...
func (c *Controller) CartoonModuleList(ctx context.Context, req *v1.CartoonModuleListReq) (res *v1.CartoonModuleListRes, err error) {
	list, total, err := moduleList(ctx, c.cartoonMod, req.Name, req.Position, req.Status, req.CategoryId, req.Page, req.Size)
	if err != nil {
		return nil, err
	}
