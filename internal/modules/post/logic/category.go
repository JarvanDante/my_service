...
func (s *sCategory) Repo(ctx context.Context) ([]*service.CategoryDTO, error) {
	...
	err := g.Model("post_category").Ctx(ctx).
		Where("site_id", postSiteId).Where("status", 1).
		OrderDesc("rank").OrderDesc("id").Scan(&list)
	if err != nil {
		return nil, err
	}
	return toCategoryDTOs(list), nil
}
...
func (s *sCategory) List(ctx context.Context, f service.CategoryFilter) ([]*service.CategoryDTO, int, error) {
	...
	m := g.Model("post_category").Ctx(ctx).Where("site_id", postSiteId)
	if f.Kind >= 0 {
	...
	return toCategoryDTOs(list), total, nil
}
...
func (s *sCategory) Create(ctx context.Context, in service.CategoryInput) (int64, error) {
	...
	cnt, err := g.Model("post_category").Ctx(ctx).
		Where("site_id", postSiteId).Where("name", in.Name).Count()
	if err != nil {
		return 0, err
	}
	if cnt > 0 {
	...
	return g.Model("post_category").Ctx(ctx).Data(g.Map{
		"site_id": postSiteId, "name": in.Name, "kind": in.Kind,
...
func (s *sCategory) Delete(ctx context.Context, id int64) error {
	...
	_, err := g.Model("post_category").Ctx(ctx).
		Where("site_id", postSiteId).Where("id", id).Delete()
	return err
}

func toCategoryDTOs(list []*entity.VideoCategory) []*service.CategoryDTO {
