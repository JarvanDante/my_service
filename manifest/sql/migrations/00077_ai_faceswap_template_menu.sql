-- 子后台 AI管理 下增加「换脸-图片模版」。
-- 接口复用 /backend/ai/templates*（固定 biz_type=1）。
-- +goose Up

INSERT INTO admin_permission (parent_id, name, route_url, component, method, icon, is_menu, hide_in_menu, affix_tab, sort)
SELECT p.id, '换脸-图片模版', '/ai/faceswap-template', 'ai/faceswap-template', '', 'lucide:scan-face', 1, 0, 0, 5
FROM admin_permission p
WHERE p.is_menu = 1 AND p.parent_id = 0 AND p.name = 'AI管理'
  AND NOT EXISTS (
      SELECT 1 FROM admin_permission x
      WHERE x.is_menu = 1 AND x.name = '换脸-图片模版' AND x.route_url = '/ai/faceswap-template'
  )
LIMIT 1;
...
CROSS JOIN (VALUES
    ('换脸模板列表', '/backend/ai/templates',     'GET',    1),
    ('新增换脸模板', '/backend/ai/templates',     'POST',   2),
    ('编辑换脸模板', '/backend/ai/templates/{id}', 'PUT',    3),
    ('删除换脸模板', '/backend/ai/templates/{id}', 'DELETE', 4)
) AS v(name, route_url, method, sort)
WHERE m.is_menu = 1 AND m.name = '换脸-图片模版' AND m.route_url = '/ai/faceswap-template'
  AND NOT EXISTS (
      SELECT 1 FROM admin_permission x
...
FROM (
    ...
    FROM admin_permission p
    WHERE (p.is_menu = 1 AND p.name = '换脸-图片模版' AND p.route_url = '/ai/faceswap-template')
       OR p.parent_id IN (
           SELECT id FROM admin_permission
           WHERE is_menu = 1 AND name = '换脸-图片模版' AND route_url = '/ai/faceswap-template'
       )
) ids
